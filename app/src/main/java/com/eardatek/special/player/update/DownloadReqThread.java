...
public class DownloadReqThread extends Thread {
    ...
    private static final String TAG = "DownloadReqThread";
        ...
        try {
            ...
            if(file.exists()) {
                ...
                while((count = input.read(buffer)) != -1){
                    ...
                    MessageHelper.sendMsg(handler, DOWNLOAD_COUNT, write_count);
                    if(getRunflag() == false){//允许结束点
                        input.close();
                        outputStream.close();
                        file.delete();
            ...
            e.printStackTrace();
            LogUtil.i(TAG,"HttpURLConnection exception");
            MessageHelper.sendMsg(handler,DOWNLOAD_ERR, e.toString());
        }
