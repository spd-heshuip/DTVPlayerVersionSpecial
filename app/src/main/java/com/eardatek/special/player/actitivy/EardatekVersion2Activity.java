...
import com.eardatek.special.player.adapter.ChanelListAdaptar;
import com.eardatek.special.player.adapter.TabFragmentPagerAdapter;
...
import com.eardatek.special.player.callback.OnTvChanelItemClickListener;
import com.eardatek.special.player.callback.ScreenListener;
import com.eardatek.special.player.data.AbstractDataProvider;
import com.eardatek.special.player.data.TvDataProvider;
import com.eardatek.special.player.fragment.SwipeWithButtonFragment;
...
import com.eardatek.special.player.util.AnimatorUtil;
import com.eardatek.special.player.util.ChannelScanner;
...
import com.eardatek.special.player.util.StringUtil;
import com.eardatek.special.player.util.WeakHandler;
import com.eardatek.special.player.util.WindowUtil;
import com.eardatek.special.player.widget.CustomEditText;
import com.eardatek.special.player.widget.CustomToast;
...
import static com.eardatek.special.player.R.id.indicator_layout;


/**
 * Created by Luke He on 16-3-30 上午10:18.
        ...
        ViewPager.OnPageChangeListener,IVideoPlayer,OnTvChanelItemClickListener,SensorEventListener
{
    ...
    private static final int SHOW_PROGRAM_NAME_LAYOUT = 6;
    private static final int HIDE_PROGRESS_WHEEL = 7;
    private static final int SHOW_NO_SIGNAL_TIPS = 8;
    private static final int USB_I2C_ERROR = 9;
    ...
    private TextView mNoSignalTipsText;
    ...
    private ChannelInfoDaoImpl mDbInstance;
...
//        setSwipeBackEnable(false);
        ...
        mNavigationHeight = WindowUtil.getNavigationBarHeight(EardatekVersion2Activity.this);
        hasNavigationBar = WindowUtil.checkDeviceHasNavagationBar(EardatekVersion2Activity.this);

        LogUtil.i(TAG,"has navigationbar:" + hasNavigationBar);
        LogUtil.i(TAG,"mNavigationHeight:" + mNavigationHeight);
        final float scale = getResources().getDisplayMetrics().density;
        LogUtil.i("EardatekVersion2","phone's density:" + scale);
...
//                isLockScreen = false;
        ...
        mDbInstance = new ChannelInfoDaoImpl(this);
...
//        mLibDTV.setDeblocking(100);
        ...
        if(ConnectSwitch.isRemoteType()){
            ...
            NetTunerCtrl.getInstance().start("127.0.0.1", 6000, "TCP", 8000, mSystemHandler, EardatekVersion2Activity.this) ;
            initUsbHelper();
        }
    }
...
//                        setSurfaceLayout(mVideoWidth, mVideoHeight, mVideoVisibleWidth, mVideoVisibleHeight, mSarNum, mSarDen);
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        //横竖屏切换时，Activity的Configuration会重新初始化，locale的默认值等于Locale.getDefault()
        //所以需要重新切换成应用设置的语言
        swtichLanguage();
    }
...
//            LogUtil.i("SwipeWithButtonFragment","Activity result channel size:" + mDataProvider.getCount());
        ...
        if (requestCode == Constants.WIFI_REQUEST_CODE && ConnectSwitch.isRemoteType()){
            String wifiName = NetworkUtil.getWifiName(getApplicationContext());
...
//        NetTunerCtrl.getInstance().stop();
    ...
    private void changeToLandscape(){
        ...
        mVideoType.setVisibility(View.GONE);

        mGoBackButton.setVisibility(View.VISIBLE);
        mCurTime.setVisibility(View.VISIBLE);
        mChanelList.setVisibility(View.VISIBLE);

        if (mNavigationHeight > 0 && hasNavigationBar){

            int mScreenHeight = getWindow().getDecorView().getHeight();

            ViewGroup.LayoutParams navigationLp = mTitleBar.getLayoutParams();

            int width = mScreenHeight - mNavigationHeight;
            LogUtil.i(TAG,"screen height:" + mScreenHeight);
            LogUtil.i(TAG,"mTitleBar width:" + width);
            navigationLp.width = width;
            navigationLp.height = LinearLayout.LayoutParams.WRAP_CONTENT;
            mTitleBar.setLayoutParams(navigationLp);
            mTitleBar.invalidate();

            navigationLp = mBottomTitleLayout.getLayoutParams();
            navigationLp.width = width;
            navigationLp.height = LinearLayout.LayoutParams.WRAP_CONTENT;
            mBottomTitleLayout.setLayoutParams(navigationLp);
            mBottomTitleLayout.invalidate();
        }
        ...
        mSurfaceContainLayout.invalidate();
    ...
    private void changeToPortRait(){
        ...
        mVideoType.setVisibility(View.VISIBLE);
        ...
        if (mNavigationHeight > 0 && hasNavigationBar){
            ViewGroup.LayoutParams navigationLp = mTitleBar.getLayoutParams();
            navigationLp.width = LinearLayout.LayoutParams.MATCH_PARENT;;
            navigationLp.height = LinearLayout.LayoutParams.WRAP_CONTENT;
            mTitleBar.setLayoutParams(navigationLp);
            mTitleBar.invalidate();
...
//            mLibDTV.setHardwareAcceleration(LibDTV.HW_ACCELERATION_DISABLED);
    ...
    private void initUsbHelper(){
        usbHelper = new UsbHelper(EardatekVersion2Activity.this) {
            boolean workingFlag = true;
            SweetAlertDialog usbAlertdialog;
            @Override
            public void usb_in() {

                UsbDevice device = usbHelper.getDeviceByPidVid(0x0131, 0x15f4);
                if (device != null && !workingFlag){
                    usbAlertdialog.changeAlertType(SweetAlertDialog.SUCCESS_TYPE);
                    usbAlertdialog.setTitleText("设备重新接入");
                    usbAlertdialog.setContentText("将跳转到初始化界面");
                    usbAlertdialog.setConfirmText("确认");
                    usbAlertdialog.setConfirmClickListener(new SweetAlertDialog.OnSweetClickListener() {
                        @Override
                        public void onClick(SweetAlertDialog sweetAlertDialog) {

                            Intent intent = new Intent(EardatekVersion2Activity.this, HomePageActivity.class);
            ...
            @Override
            public void usb_out() {
                if (usbHelper.getDeviceByPidVid(0x0131, 0x15f4) == null && workingFlag) {
                    stopPlaying();
                    mLibDTV.stop();
                    workingFlag = false;
                    usbAlertdialog = new SweetAlertDialog(EardatekVersion2Activity.this, SweetAlertDialog.WARNING_TYPE);
                    usbAlertdialog.setTitleText("设备被拨出");
                    usbAlertdialog.setContentText("设备已经被拔出，等待重连...");
                    usbAlertdialog.setConfirmText("直接退出");
                    usbAlertdialog.setConfirmClickListener(new SweetAlertDialog.OnSweetClickListener() {
                        @Override
                        public void onClick(SweetAlertDialog sweetAlertDialog) {
                            doClean();
                    ...
                    usbAlertdialog.setCancelText("取消");
                    usbAlertdialog.setCanceledOnTouchOutside(true);
                    usbAlertdialog.setCancelable(true);
                    usbAlertdialog.show();
                }
    ...
    private void initTitlebar(){
        ...
        mSuggestText = (TextView) findViewById(R.id.tips_handle);
        mNoSignalTipsText = (TextView) findViewById(R.id.no_signal_tips);
...
//        mPercentText.setText((mVolume / mMaxVolume * 100) + "%");
        ...
        mBottomTitleLayout.setAlpha(0.7f);
        ...
        mVideoType = (TextView) findViewById(R.id.video_type);
        mChanelList.setVisibility(View.GONE);

        mTitleBar.setAlpha(0.7f);
        ...
        search_ad.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!TextUtils.isEmpty(text) && isNumber){
                    ...
                    String tvType = NetTunerCtrl.getInstance().getTvType();
                    boolean isValid = ChannelScanner.isValidFreq(tvType,freq);
                    if (!isValid)
                        return;
                    if (dialog.isShowing())
                        dialog.dismiss();
...
//        mTabs.add(tab1);
    ...
    public void _lockChannel(String location) {
        ...
        try {
            media = channelInfoDao.getChannelInfo(location);
        } catch (SQLException e) {
            e.printStackTrace();
        }
        ...
        Message message = mHandler.obtainMessage(SET_TITLE);
...
//            isRadio = ":no-video";
        ...
        LogUtil.i(TAG,"program:" + mLibDTV.getProgram());
        isGetPidList = true;
    }
    ...
    public void setSurfaceLayout(int width, int height, int visible_width, int visible_height, int sar_num, int sar_den) {
        ...
        if (mLocked && mPlaying && !isSwitching){
            mHandler.sendEmptyMessage(HIDE_PROGRESS_WHEEL);
        }
...
//                    activity.playbackEndReached();
                ...
                case EventHandler.HardwareAccelerationError:
                    ...
                    activity.showInfo(DTVApplication.getAppResources().getString(R.string.harddecodeerror), 500);
                    activity.mReplay = true;
...
//                    activity.mLibDTV.setHardwareAcceleration(LibDTV.HW_ACCELERATION_DISABLED);
    ...
    private static final class CheckLockThread implements Runnable{
        ...
        CheckLockThread(EardatekVersion2Activity activity) {
...
//                    activity.mHandler.sendEmptyMessage(USB_I2C_ERROR);
//                    return;
                }
                try {
                    Thread.sleep(3000);
    ...
    private static final class MyRunable implements Runnable{
        ...
        MyRunable(EardatekVersion2Activity activity) {
        ...
        public void run() {
            ...
            while (activity.mPlaying) {
                ...
                activity.mHandler.sendEmptyMessage(SHOW_PROGRAM_NAME_LAYOUT);
...
//                    LogUtil.i(TAG,activity.mLocked + "" + lockFailedCount  + "" + activity.mReplay + activity.isGetPidList);
                ...
                if (!activity.mLocked || !activity.isGetPidList){
                    isSwitching = false;
                    activity.mHandler.sendEmptyMessage(HIDE_PROGRESS_WHEEL);
                    activity.mHandler.sendEmptyMessage(SHOW_NO_SIGNAL_TIPS);
                    activity.mPlaying = false;
                    return;
                }
    ...
    private static final class SignalStatusRunnable implements Runnable {
        ...
        SignalStatusRunnable(EardatekVersion2Activity activity) {
            this.mActivity = new WeakReference<>(activity);
        }
    ...
    public final static class MyHandler extends Handler {
        ...
        MyHandler(EardatekVersion2Activity activity) {
            this.mActivity = new WeakReference<>(activity);
        }
        @Override
        public void handleMessage(Message msg) {
    ...
    private void showHideTitleBar(boolean bShow) {
        ...
        if (bShow) {
            ...
            if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT)
                ...
                AnimatorUtil.alphaAnimatorIn(mIsLock,0.0f,1.0f,300);
...
//            AnimatorUtil.animateIn(mBottomTitleLayout,100f,0f,300);
            AnimatorUtil.alphaAnimatorIn(mTitleBar,0.0f,1.0f,300);
            AnimatorUtil.alphaAnimatorIn(mBottomTitleLayout,0.0f,1.0f,300);
            mPlay.setVisibility(View.INVISIBLE);
            mHandler.sendEmptyMessageDelayed(FADE_OUT, 5000);
        } else {
            AnimatorUtil.alphaAnimatorIOut(mIsLock,1.0f,0.0f,300);
//            mIsLock.setVisibility(View.INVISIBLE);
//            AnimatorUtil.animateOut(mTitleBar,-100.f,300);
//            AnimatorUtil.animateOut(mTitleBar,-mTitleBar.getHeight(),300);
            AnimatorUtil.alphaAnimatorIOut(mTitleBar,1.0f,0.0f,300);
            if (mPlaying)
                mPlay.setVisibility(View.INVISIBLE);
            if(mChanelList.getScrollState() == RecyclerView.SCROLL_STATE_IDLE){
//                AnimatorUtil.animateOut(mBottomTitleLayout,100f,300);
                    AnimatorUtil.alphaAnimatorIOut(mBottomTitleLayout,1.0f,0.0f,300);
            }
            else{
                mHandler.removeMessages(FADE_OUT);
                mHandler.sendEmptyMessageDelayed(FADE_OUT, 5000);
    ...
    private static class LiveTvPlayerHandler extends WeakHandler<EardatekVersion2Activity> {
        LiveTvPlayerHandler(EardatekVersion2Activity owner) {
            super(owner);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case SET_TITLE:
                    ...
                    activity.mVideoType.setText(String.format("%s%s",
                            DTVApplication.getAppResources().getText(R.string.videocode), type));
                    if (!activity.mNoSignalTips.isShown()){
                        activity.mProgramName.setVisibility(View.VISIBLE);
                        activity.mProgramName.setText(activity.mServiceName);
                    }
                    break;
                case SHOW_PROGRAM_NAME_LAYOUT:
                    activity.showHideTitleBar(false);
                ...
                case USB_I2C_ERROR:
                    ...
                    activity.mNoSignalTipsText.setText(activity.getString(R.string.i2c_error));
                    activity.mSuggestText.setText("");
...
//                    activity.PlayOrPauseOptionEvent();
                ...
                case SHOW_NO_SIGNAL_TIPS:
                    ...
                    activity.mPlay.setVisibility(View.INVISIBLE);
                    activity.mNoSignalTips.setVisibility(View.VISIBLE);
                    activity.mNoSignalTipsText.setText(activity.getString(R.string.no_signal_tips));
                    activity.mSuggestText.setText(activity.getString(R.string.ant_tips));
                    removeMessages(SHOW_NO_SIGNAL_TIPS);
                    break;
        ...
        public boolean onMenuItemClick(MenuItem item) {
            switch (item.getItemId()){
                case R.id.edit_list:
                    ...
                    try {
                        lists = mDbInstance.getAllChannelInfo();
                    } catch (SQLException e) {
                        e.printStackTrace();
                    }
                    if (lists.size() > 0)
...
//                        CustomToast.showToast(DTVApplication.getAppContext(),"No channel to delte",3000);
                    break;
                case R.id.setting:
                    SettingActivity.startSettingActivity(EardatekVersion2Activity.this,mFreq);
                    EardatekVersion2Activity.this.overridePendingTransition(R.anim.photo_dialog_in_anim,
                            R.anim.photo_dialog_out_anim);
                    break;
                default:
...
//        mPercentText.setText(String.format(Locale.ENGLISH,"%d%%",(vol * 10 / mMaxVolume) * 10));
        ...
        mPercentText.setText(String.format(Locale.ENGLISH,"%d%%", percentage));
...
//                onBrightnessSlide((mOldY-y));
    ...
    private void changeSurfaceLayout() {
        int sw;
        int sh;
//        LogUtil.i(TAG,"changeSurfaceLayout");
        // get screen size
        sw = getWindow().getDecorView().getWidth();
        sh = getWindow().getDecorView().getHeight();
//        LogUtil.i(TAG,"screen width:" + getWindow().getDecorView().getWidth());
//        LogUtil.i(TAG,"screen height:" + getWindow().getDecorView().getHeight());
        if (mLibDTV != null)
        ...
        if (dw * dh == 0 || mVideoWidth * mVideoHeight == 0) {
            LogUtil.i(TAG,"mVideoWidth * mVideoHeight == 0");
            if (isPortrait){
                ViewGroup.LayoutParams lp = mSurfaceContainLayout.getLayoutParams();
                lp.width = LinearLayout.LayoutParams.MATCH_PARENT;
                lp.height = getResources().getDimensionPixelSize(R.dimen.surface_height);
                mSurfaceContainLayout.setLayoutParams(lp);
    ...
    private void getProgramInfo(int dx){
        ...
        try {
            info = mDbInstance.getChannelInfo(mLocation);
        } catch (SQLException e) {
            e.printStackTrace();
        }
        if (info != null)
            mServiceName = info.getTitle();
        mHandler.sendEmptyMessage(SET_TITLE);
...
//            return false;
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (isSwitching){
            CustomToast.showToast(getApplicationContext(),getResources().getString(R.string.switching),1000);
            return true;
        }
...
//                LogUtil.i(TAG,"isAutoRotate: " + isAutoRotate);
                if (!isAutoRotate)
                    return;
                // 设置竖屏
                if (((rotation >= 0) && (rotation <= 45)) || (rotation > 315)) {
                    if (mClick) {
                        ...
                        if (mIsLand ) {
                            LogUtil.i(TAG,"change to  portrait");
                            setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
                            changeToPortRait();
                            isAutoRotate = false;
                ...
                // 设置横屏
                    ...
                    if (mClick) {
                        ...
                        if (!mIsLand && mPlaying ) {
                            LogUtil.i(TAG,"change to  landscape");
                            setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
                            changeToLandscape();
                            isAutoRotate = false;
                            mHandler.sendEmptyMessageDelayed(AUTO_ROTATE_DELAY,1500);
                    ...
                    if (mClick) {
                        ...
                        if (!mIsLand && mPlaying) {
                            if (getRequestedOrientation() != ActivityInfo.SCREEN_ORIENTATION_REVERSE_LANDSCAPE){
                                LogUtil.i(TAG,"change to reverse landscape");
        ...
        mWifiReceiver = new NetworkUtil.NetWorkConnectChangedReceiver(new NetworkUtil.NetWorkConnectChangedReceiver.WifiStateChangedListener() {
            ...
            public void onDisabled() {
                if (mLibDTV.isPlaying()){
                    ...
                    if (!saveName.equals(wifiName) && !NetTunerCtrl.isConnectToDevice(wifiName)){
                            showWifiDialog();
                    }else if(!saveName.equals(wifiName) && NetTunerCtrl.isConnectToDevice(wifiName)){
